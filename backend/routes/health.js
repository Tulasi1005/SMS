...
// Update existing health record
router.put("/health-records/:id", authMiddleware, restrictAccess, async (req, res) => {
    try {
      const {
        studentId,
        admissionNo,
      ...
      // Handle flat values from frontend and map to schema structure
      const updatedFields = {
        studentId: studentId || healthRecord.studentId,
        admissionNo: admissionNo || healthRecord.admissionNo,
        height:
          height !== undefined
        ...
        weight:
          weight !== undefined
            ...
            : healthRecord.weight,
        bloodGroup: bloodGroup !== undefined ? bloodGroup : healthRecord.bloodGroup,
        allergies:
          allergies !== undefined
        ...
        chronicConditions:
          chronicConditions !== undefined
            ...
            : healthRecord.chronicConditions,
        medications:
        ...
        lastCheckup:
          lastCheckup !== undefined
            ? typeof lastCheckup === "object"
              ? lastCheckup
              : { date: new Date(lastCheckup) }
            : healthRecord.lastCheckup,
        updatedAt: Date.now(),
      };
  
      // Input validation
      if (updatedFields.height && (isNaN(updatedFields.height.value) || updatedFields.height.value < 0)) {
        return res.status(400).json({ message: "Height must be a positive number" });
      }
      if (updatedFields.weight && (isNaN(updatedFields.weight.value) || updatedFields.weight.value < 0)) {
        return res.status(400).json({ message: "Weight must be a positive number" });
      }
      if (
        updatedFields.bloodGroup &&
        !["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].includes(updatedFields.bloodGroup)
      ) {
        return res.status(400).json({ message: "Invalid blood group" });
      }
  
      Object.assign(healthRecord, updatedFields);
