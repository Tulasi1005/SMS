...
const ParentDashboard = () => {
  ...
  const fetchChildData = async (childId) => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/students/${childId}`, {
        headers: {
              ...
              <Grid item xs={12} md={6}>
                ...
                >
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                    ...
                    >
                      <CalendarToday
                        sx={{ mr: 1, fontSize: { xs: 20, sm: 22, md: 24 }, color: "#6366f1" }}
                      />
                  ...
                  <Box sx={{ display: "flex", justifyContent: "center", gap: 2, mt: 1.5, p: 1, background: "rgba(255, 255, 255, 0.7)" }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                      <Box
                        sx={{ width: 6, height: 6, borderRadius: "50%", background: "linear-gradient(145deg, #6366f1, #a855f7)" }}
                      />
